...
def process_exceptions(po: ExtractedPO, repo) -> list[Issue]:
    """Check item existence against item master.

    Backfill stock and auto-commit. Mutates line_items.
    """
    issues: list[Issue] = []
    for idx, li in enumerate(po.line_items):
        ...
        if (
            not li.customer_item_number
            and li.item_number
            and repo.find_item(li.item_number) is None
        ):
        ...
        if li.customer_item_number:
            li.item_number = (
                repo.resolve_customer_item(po.header.customer or "", li.customer_item_number) or ""
            )
