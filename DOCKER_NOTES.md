...
## HTTP 状态码分类，以及"401 是不是代表出错了"

**四大类**：
- **2xx** —— 成功类，服务器按要求处理完了（200 最常见）
- **3xx** —— 重定向类，"你要的东西不在这，去另一个地址"
- **4xx** —— **客户端错误**类，服务器认为"你的请求本身有问题"（没登录、没权限、格式不对、资源不存在）
- **5xx** —— 服务器错误类，服务器自己出问题了（代码抛异常、崩溃）

401 属于 4xx，标准分类上确实叫"客户端错误"（client error）。

**但"状态码的分类叫错误"，不等于"这次测试的结果是错的"**——容易混淆的地方：

- 我们**故意不带密码**去请求，目的就是验证"门禁挡不挡得住没授权的人"。这种情况下拿到 401，恰恰说明系统**按设计正常工作**，是一次**成功的测试**（验证了正确的行为），不是"系统坏了"。
- 真正代表"出问题了"的情况是：**带着正确的密码去请求，结果还是 401**——那才说明门禁逻辑有 bug（比如密码没生效）。

所以准确的说法是：401/200 只是"服务器如实反映了这次请求的处理结果"——不带对密码就该 401、带对密码就该 200，两次都拿到预期结果，说明验证是**双重成功**，不是"一个错一个对"。

## HTTPS 跳转看到的 301 vs 308，区别是什么

给 nginx 配置 HTTP → HTTPS 自动跳转后，用 `curl -s -o /dev/null -w "%{http_code}"` 测未加密的 `http://` 地址，会看到 3xx（比如 301 或 308），这两个都是"永久重定向"，语义上很接近，区别在于对请求方法/请求体的处理严格程度：

- **301 Moved Permanently**——很老的标准，历史上不同客户端对"重定向后要不要保留原来的请求方法（比如 POST）"处理不一致，有些老客户端收到 301 后会**擅自把 POST 改成 GET** 再发一次，等于请求体（比如上传的文件）丢了。
- **308 Permanent Redirect**——语义跟 301 一样是"永久移动"，但**明确规定**：重定向后必须用**同样的方法、同样的请求体**重新发一次，不允许降级成 GET。

**这个区别对这个项目是有实际意义的**：`/api/process` 是个 **POST** 接口（上传 PDF 文件）。如果重定向机制处理不当，极端情况下 POST 上传请求可能被错误降级成 GET，导致文件丢失、请求出错——308 从协议层面就杜绝了这个可能性。
